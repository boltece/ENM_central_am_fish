...
#setwd(\"./Output 3 with Projection Layer-taeda/\")
t_background <-read.csv(file="Pinus_taeda_0_backgroundPredictions.csv\")
taeda_data<- read.csv(file="taeda_data.csv")
                        files<-list.files(".",full.names=T)
                        files
                        bilFile<-files[grep(pattern=".bil",x=files)]
                        clim <- list.files(pattern = ".bil",full.names = TRUE)
                        Data_t <- taeda_data[,1:2]
                        clim
                        Data_t
...
write.csv(Corplot1, file="Pha_bioclim_correlations.csv")
                        ...
                        install.packages("corrplot")
                        library(corrplot)
                        Corplot <- cor(Data_t, use="pairwise.complete.obs")
                        Corplot
                        corrplot(Corplot)
                        

bio19cUTM <- crop(x=bio19, y=ext2)
writeRaster(x=bio19c, file="bio19c.grd", overwrite=TRUE)
